# duocolor/ — Untitled UI Pro Icons 4K Synth

**3,207** SVGs in this folder. 24×24 viewBox, `stroke="currentColor"` or `fill="currentColor"`, render at any pixel size.

## Style definition

Two-colour glyphs painted with `--uip-icon-primary` and `--uip-icon-secondary` CSS custom properties. The secondary layer also has 0.35 opacity.

```html
<div style="--uip-icon-primary: var(--uip-brand-600); --uip-icon-secondary: var(--uip-accent-500);">
  <img src="duocolor/lu-shield-check.svg" width="20" height="20"/>
</div>
```
