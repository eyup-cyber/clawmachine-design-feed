// Catalyst-style Badge + BadgeButton — recreated from the scraped API at
...
const colors: Record<string, string> = {
  ...
  indigo:  'bg-indigo-500/15 text-indigo-700 group-data-hover:bg-indigo-500/25 dark:bg-indigo-500/10 dark:text-indigo-400 dark:group-data-hover:bg-indigo-500/20',
  violet:  'bg-violet-500/15 text-violet-700 group-data-hover:bg-violet-500/25 dark:bg-violet-500/10 dark:text-violet-400 dark:group-data-hover:bg-violet-500/20',
...
type BadgeProps = { color?: BadgeColor; className?: string; children: React.ReactNode } & React.HTMLAttributes<HTMLSpanElement>
export function Badge({ color = 'zinc', className, ...props }: BadgeProps) {
  return <span {...props} className={clsx('inline-flex items-center gap-x-1.5 rounded-md px-1.5 py-0.5 text-sm/5 font-medium sm:text-xs/5 forced-colors:outline', colors[color], className)} />
}

type BadgeButtonProps = { color?: BadgeColor; className?: string; children: React.ReactNode } & (Omit<Headless.ButtonProps, 'className' | 'children'> | Omit<React.ComponentPropsWithoutRef<typeof Link>, 'className' | 'children'>)

export const BadgeButton = forwardRef(function BadgeButton(
  { color = 'zinc', className, children, ...props }: BadgeButtonProps,
  ref: React.ForwardedRef<HTMLElement>
) {
  const classes = clsx(className, 'group relative inline-flex rounded-md focus:outline-hidden data-focus:outline-2 data-focus:outline-offset-2 data-focus:outline-blue-500')
  return 'href' in props ? (
    <Link {...(props as React.ComponentPropsWithoutRef<typeof Link>)} className={classes} ref={ref as React.ForwardedRef<HTMLAnchorElement>}>
      <TouchTarget><Badge color={color}>{children}</Badge></TouchTarget>
    </Link>
  ) : (
